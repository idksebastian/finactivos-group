...
/** Formato revista: una etapa abierta a la vez, con transición suave de alto. */
function ProcessAccordion() {
  const [openIndex, setOpenIndex] = useState(0);
  let counter = 0;

  return (
    <div className="divide-y divide-fin-line border-y border-fin-line">
      {stages.map((s, i) => {
        const isOpen = openIndex === i;
        const startCount = counter;
        counter += s.steps.length;

        return (
          <div key={s.stage} id={slugify(s.stage)} className="scroll-mt-24">
            <button
              type="button"
            ...
            >
              <div className="flex items-center gap-5">
                <span
                  className={`flex h-12 min-w-12 items-center justify-center rounded-[3px] border-2 px-2 font-display text-lg font-bold transition-colors ${
                    isOpen
                      ? "border-fin-lime bg-fin-lime text-fin-teal"
                      : "border-fin-line text-fin-ink/40"
                  }`}
                >
                  {String(i + 1).padStart(2, "0")}
                </span>
                <div>
                  <p className={`${ds.h2} ${isOpen ? "text-fin-teal" : "text-fin-ink/50"}`}>
                    {s.stage}
                  </p>
                  <p className={`mt-1 ${ds.timeBadge}`}>{s.time}</p>
                </div>
              </div>
              <svg
                viewBox="0 0 16 16"
                fill="none"
                className={`h-4 w-4 shrink-0 stroke-fin-teal transition-transform duration-300 ${
                  isOpen ? "-rotate-180" : ""
                }`}
                aria-hidden="true"
              >
            ...
            >
              <div className="overflow-hidden">
                <ol className="divide-y divide-fin-line border-t border-fin-line pb-10 md:ml-17">
                  {s.steps.map((st, j) => (
